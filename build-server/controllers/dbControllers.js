...
const dbControllers = {
  ...
  async getSettings() {
    try {
      ...
      console.log(
        'Repo settings successfully got: ',
        response.data.data.repoName,
        response.data.data.buildCommand
      );
  ...
  async getBuildsList() {
    try {
      ...
      if (response.data.data.length) {
        // если список билдов не пустой - сохраняем его для дальнейшей работы
        console.log('Build list successfully got');
        storage.updateBuildsList(response.data.data);
      } else {
        // если пустой - потвторяем запрос к БД позже
      ...
      const interval =
        apiBDErrorsCount <= maxApiBDErrorsCount ? shortInterval : longInterval;
  ...
  async startBuild(params) {
    try {
      ...
      console.error(
        'Can not set status In Progress because of en Error: ',
        err
      );
    }
  },
